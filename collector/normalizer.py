...
def normalize_event(event: dict) -> dict | None:
    ...
    try:
        ...
        logger.warning("Skipping unparseable CloudWatch event %s", event.get("eventId"))
        return None

    missing = [f for f in REQUIRED_FIELDS if f not in payload]
    if missing:
        logger.warning("Skipping event %s missing fields %s", event.get("eventId"), missing)
        return None

    try:
        timestamp = datetime.fromisoformat(payload["timestamp"])
    except (TypeError, ValueError):
        logger.warning("Skipping event %s with unparseable timestamp", event.get("eventId"))
        return None
